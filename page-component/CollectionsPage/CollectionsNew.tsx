...
export const CollectionsNew = ({ take, isUpdated }: { take: number; isUpdated: boolean }) => {
	...
	const getCollections = async (take: number, skip: number) => {
		...
		const response = await fetch(`${API.collections.getNew}?take=${take}&skip=${skip}`, { credentials: 'include', method: 'get' });
