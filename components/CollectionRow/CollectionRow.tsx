import { Collection, IsTruthy } from '@/components';
...
export const CollectionRow = ({ collections, wrapperClassName, ...props }: CollectionRowProps) => {
	...
	return (
		<div className={cn(styles.row, wrapperClassName)}>
			{useMemo(
				() => collections.map((collection, index) => <Collection key={collection.id + index * Math.random()} info={collection} />),
				[collections]
			)}
			<IsTruthy condition={collections.length == 0}>
				{new Array(collectionsInRow).fill(<></>).map((_, i) => (
					<Collection {...props} key={Math.random() * i} />
				))}
			</IsTruthy>
		</div>
	);
